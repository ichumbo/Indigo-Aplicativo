...
exports.PROTOCOL_CATALOG = exports.GIRTH_SITE_LABELS = exports.SKINFOLD_SITE_LABELS = void 0;
exports.getProtocolDefinition = getProtocolDefinition;
exports.getProtocolFields = getProtocolFields;
exports.parseDecimalInput = parseDecimalInput;
exports.classifyAdultBmi = classifyAdultBmi;
exports.getProtocolApplicability = getProtocolApplicability;
exports.consolidateSkinfoldMeasurement = consolidateSkinfoldMeasurement;
exports.calculateCompositionProtocol = calculateCompositionProtocol;
exports.SKINFOLD_SITE_LABELS = {
    chest: "Peitoral",
...
const FIELD_LIBRARY = {
    chest: {
        id: "chest",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.chest,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    midaxillary: {
        id: "midaxillary",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.midaxillary,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    triceps: {
        id: "triceps",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.triceps,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    subscapular: {
        id: "subscapular",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.subscapular,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    abdominal: {
        id: "abdominal",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.abdominal,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    suprailiac: {
        id: "suprailiac",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.suprailiac,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    thigh: {
        id: "thigh",
        kind: "skinfold",
        label: exports.SKINFOLD_SITE_LABELS.thigh,
        unit: "mm",
        repeatable: true,
        min: 1,
        max: 100,
    ...
    abdomenMeanCm: {
        id: "abdomenMeanCm",
        kind: "girth",
        label: exports.GIRTH_SITE_LABELS.abdomenMeanCm,
        unit: "cm",
        min: 40,
        max: 250,
        suspiciousMin: 55,
...
exports.PROTOCOL_CATALOG = [
    {
        id: "jackson-pollock-3",
        displayName: "Jackson & Pollock — 3 dobras",
        shortName: "JP 3 dobras",
        author: "Jackson, Pollock; Jackson, Pollock & Ward",
        reference: "Jackson AS, Pollock ML. Generalized equations for predicting body density of men. Br J Nutr. 1978; Jackson AS, Pollock ML, Ward A. Generalized equations for predicting body density of women. Med Sci Sports Exerc. 1980.",
    ...
    },
    {
        ...
        author: "Jackson, Pollock; Jackson, Pollock & Ward",
        reference: "Jackson AS, Pollock ML. Br J Nutr. 1978; Jackson AS, Pollock ML, Ward A. Med Sci Sports Exerc. 1980.",
        referenceUrl: "https://pubmed.ncbi.nlm.nih.gov/718832/",
    ...
    },
    {
        ...
        shortName: "Guedes",
        author: "Guedes",
        reference: "Guedes DP. Estudo da gordura corporal através da mensuração dos valores de densidade corporal e espessura de dobras cutâneas em universitários. Dissertação, UFSM, 1985.",
    ...
    },
    {
        ...
        reference: "Weltman A, Seip RL, Tran ZV. Practical assessment of body composition in adult obese males. Human Biology. 1987; Weltman A, Levine S, Seip RL, Tran ZV. Accurate assessment of body composition in obese females. Am J Clin Nutr. 1988.",
    ...
    },
    {
        ...
        reference: "Resultados dependem do fabricante/modelo e algoritmo proprietário do equipamento; o app não reproduz fórmula genérica.",
...
const REQUIRED_BIO_FIELDS = new Set(["bodyFatPercent"]);
...
function getProtocol(protocolId) {
    return exports.PROTOCOL_CATALOG.find((protocol) => protocol.id === protocolId);
}
function getProtocolDefinition(protocolId) {
    return getProtocol(protocolId);
}
function getProtocolFields(protocolId, sex) {
...
function classifyAdultBmi(bmi, ageYears) {
    if (typeof bmi !== "number")
        return undefined;
    if (typeof ageYears === "number" && ageYears < 20)
        return "Tabela adulta não aplicada";
    if (bmi < 18.5)
        return "Baixo peso";
    if (bmi < 25)
        return "Eutrofia";
...
function getProtocolApplicability(protocolId, input) {
    const protocol = getProtocol(protocolId);
    if (!protocol)
        return { disabled: true, reasons: ["Protocolo não encontrado."], warnings: [] };
    const reasons = [];
    const warnings = [];
...
function consolidateSkinfoldMeasurement(measurement) {
    ...
    if (typeof measurement?.consolidatedMm === "number" && Number.isFinite(measurement.consolidatedMm)) {
        ...
        return {
            ...
            consolidation: "valor-consolidado",
        };
    }
    if (valid.length === 0)
        return undefined;
    ...
    return {
        ...
        spread,
        validAttempts: valid.length,
        consolidation: "media-aritmetica",
    };
}
function siriFromDensity(density) {
    return 495 / density - 450;
}
function getNumberField(input, field) {
    if (field.kind === "skinfold") {
        return consolidateSkinfoldMeasurement(input.measurements?.skinfolds?.[field.id])?.value;
    }
    if (field.kind === "girth") {
        return input.measurements?.girthsCm?.[field.id];
    }
    if (field.kind === "bioimpedance") {
        const value = input.measurements?.bioimpedance?.[field.id];
        return typeof value === "number" ? value : undefined;
    }
    return undefined;
}
function validateNumericField(field, value, errors, warnings) {
    ...
    if (typeof field.min === "number" && value < field.min)
        errors.push(`${field.label} abaixo do limite mínimo (${field.min}).`);
    if (typeof field.max === "number" && value > field.max)
        errors.push(`${field.label} acima do limite máximo (${field.max}).`);
...
function validateBase(input, errors, warnings) {
    if (input.protocolId !== "bioimpedance" && !input.ageYears)
        errors.push("Idade na data da avaliação é obrigatória para este protocolo.");
    if (!input.weightKg)
    ...
    if (typeof input.weightKg === "number") {
        ...
        if (input.weightKg < 25 || input.weightKg > 350)
            warnings.push("Peso fora da faixa usual; confira o valor.");
    }
    if (typeof input.heightCm === "number") {
        if (input.heightCm <= 0)
            errors.push("Altura deve ser maior que zero.");
        if (input.heightCm < 90 || input.heightCm > 230)
            warnings.push("Altura fora da faixa usual; confira o valor.");
    }
...
function getConsolidatedMeasurements(input, fields) {
    return fields.map((field) => {
        if (field.kind === "skinfold") {
            const measurement = input.measurements?.skinfolds?.[field.id];
        ...
        if (field.kind === "girth") {
            return {
                ...
                label: field.label,
                unit: field.unit,
                value: input.measurements?.girthsCm?.[field.id],
            };
        }
        const bioValue = input.measurements?.bioimpedance?.[field.id];
        return {
            fieldId: field.id,
            label: field.label,
            unit: field.unit,
...
function comparePrevious(current, previous, protocolId) {
    if (!previous?.results.bodyFatPercent || !current.bodyFatPercent)
        return undefined;
    ...
    return {
        ...
        fatMassKgDelta: typeof current.fatMassKg === "number" && typeof previous.results.fatMassKg === "number"
            ? round(current.fatMassKg - previous.results.fatMassKg, 1)
            : undefined,
    };
}
function calculateCompositionProtocol(input, previous) {
    ...
    fields.forEach((field) => {
        if (field.kind === "bioimpedance" && !field.required)
            return;
    ...
    const spec = protocol.sexSpecs[input.sex];
    if (!spec)
        errors.push("Protocolo sem equação para o sexo informado.");
    ...
    if (errors.length === 0 && spec) {
        if (["jackson-pollock-3", "jackson-pollock-7", "guedes-3", "faulkner-4"].includes(protocol.id)) {
            const sum = spec.fields.reduce((total, fieldId) => total + (getNumberField(input, FIELD_LIBRARY[fieldId]) ?? 0), 0);
    ...
    const fatMassKg = typeof bioFatMass === "number"
        ? round(bioFatMass, 1)
        : input.weightKg && typeof bodyFatPercent === "number"
            ? round(input.weightKg * bodyFatPercent / 100, 1)
            : undefined;
    const leanMassKg = typeof bioLeanMass === "number"
        ? round(bioLeanMass, 1)
        : input.weightKg && typeof fatMassKg === "number"
            ? round(input.weightKg - fatMassKg, 1)
            : undefined;
    const targetDecimal = typeof input.targetBodyFatPercent === "number" && input.targetBodyFatPercent > 0 && input.targetBodyFatPercent < 80
    ...
    const fatMassToChangeKg = typeof targetFatMassKg === "number" && typeof fatMassKg === "number" ? round(targetFatMassKg - fatMassKg, 1) : undefined;
    const results = {
        bodyFatPercent,
