...
exports.listConversationsForTrainer = listConversationsForTrainer;
exports.sendChatMessage = sendChatMessage;
exports.markConversationAsRead = markConversationAsRead;
exports.getUnreadChatCountForUser = getUnreadChatCountForUser;
exports.sendTrainerAnnouncement = sendTrainerAnnouncement;
exports.resetChatStoreForTests = resetChatStoreForTests;
...
function getConversationId(trainerId, studentId) {
    return `conv_${trainerId}_${studentId}`;
}
const initialConversationId = getConversationId(feedback_store_1.DEMO_TRAINER.id, feedback_store_1.DEMO_STUDENT.id);
const initialMessages = [
    {
        id: "msg-init-1",
        conversationId: initialConversationId,
        senderId: feedback_store_1.DEMO_TRAINER.id,
        senderName: feedback_store_1.DEMO_TRAINER.name,
        senderRole: "TRAINER",
        senderAvatar: undefined,
        receiverId: feedback_store_1.DEMO_STUDENT.id,
        receiverName: feedback_store_1.DEMO_STUDENT.name,
    ...
    },
    {
        id: "msg-init-2",
        conversationId: initialConversationId,
        senderId: feedback_store_1.DEMO_STUDENT.id,
        senderName: feedback_store_1.DEMO_STUDENT.name,
        senderRole: "STUDENT",
        senderAvatar: feedback_store_1.DEMO_STUDENT.avatar,
        receiverId: feedback_store_1.DEMO_TRAINER.id,
        receiverName: feedback_store_1.DEMO_TRAINER.name,
...
async function readChatState() {
    try {
        const stored = await async_storage_1.default.getItem(CHAT_STORAGE_KEY);
        if (!stored) {
            await async_storage_1.default.setItem(CHAT_STORAGE_KEY, JSON.stringify(defaultState));
            return defaultState;
        }
...
async function writeChatState(state) {
    await async_storage_1.default.setItem(CHAT_STORAGE_KEY, JSON.stringify(state));
}
/**
 * Obtém ou inicializa a conversa entre um treinador e um aluno
 */
async function getOrCreateConversation(trainerId = feedback_store_1.DEMO_TRAINER.id, studentId = feedback_store_1.DEMO_STUDENT.id, trainerName = feedback_store_1.DEMO_TRAINER.name, studentName = feedback_store_1.DEMO_STUDENT.name, trainerAvatar, studentAvatar) {
 ...
 */
async function listConversationsForTrainer(trainerId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readChatState();
    const convs = Object.values(state.conversations).filter((c) => c.trainerId === trainerId);
...
async function sendChatMessage(input) {
    ...
    const convId = input.conversationId ||
        getConversationId(input.senderRole === "TRAINER" ? input.senderId : input.receiverId, input.senderRole === "STUDENT" ? input.senderId : input.receiverId);
    const now = new Date().toISOString();
    const displayText = cleanText || (input.mediaType === "image"
        ? "Foto"
    ...
    try {
        await (0, feedback_store_1.createWorkoutNotification)({
            ...
            title: input.senderRole === "TRAINER"
 ...
 */
async function markConversationAsRead(conversationId, readerUserId) {
    const state = await readChatState();
 ...
 */
async function getUnreadChatCountForUser(userId, role) {
    const state = await readChatState();
 ...
 */
async function sendTrainerAnnouncement(input) {
    const targetStudentId = input.studentId || feedback_store_1.DEMO_STUDENT.id;
    const targetStudentName = input.studentName || feedback_store_1.DEMO_STUDENT.name;
    const tId = input.trainerId || feedback_store_1.DEMO_TRAINER.id;
    const tName = input.trainerName || feedback_store_1.DEMO_TRAINER.name;
 ...
 */
async function resetChatStoreForTests() {
