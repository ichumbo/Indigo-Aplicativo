...
exports.CARDIO_PROTOCOL_CATALOG = exports.CARDIO_PROTOCOL_CATEGORIES = void 0;
exports.getCardioProtocolDefinition = getCardioProtocolDefinition;
exports.createCardioExecution = createCardioExecution;
exports.parseCardioDecimal = parseCardioDecimal;
exports.generateCardioStages = generateCardioStages;
exports.detectConconiDeflection = detectConconiDeflection;
exports.calculateCardioProtocolSnapshot = calculateCardioProtocolSnapshot;
exports.CARDIO_PROTOCOL_CATEGORIES = [
...
exports.CARDIO_PROTOCOL_CATALOG = [
    {
        id: "cooper-12min",
    ...
    },
    {
        ...
        formula: "VO₂máx = 132,853 - 0,0769*peso(lb) - 0,3877*idade + 6,315*sexoMasculino - 3,2649*tempo(min) - 0,1565*FCfinal",
...
function getCardioProtocolDefinition(protocolId) {
    return exports.CARDIO_PROTOCOL_CATALOG.find((protocol) => protocol.id === protocolId);
}
function createCardioExecution(protocolId, order) {
    const protocol = getCardioProtocolDefinition(protocolId);
    if (!protocol)
        throw new Error("Protocolo cardiorrespiratório não encontrado.");
...
function parseCardioDecimal(value) {
...
function generateCardioStages(protocolId, config, count) {
    const stages = [];
    for (let index = 0; index < count; index += 1) {
        stages.push({
            ...
            speedKmh: protocolId === "conconi-treadmill"
                ? round((config.initialSpeedKmh ?? 8) + index * (config.speedIncrementKmh ?? 0.5), 2)
                : undefined,
            powerWatts: protocolId === "conconi-bike" && typeof config.initialPowerWatts === "number"
                ? round(config.initialPowerWatts + index * (config.powerIncrementWatts ?? 20), 1)
                : undefined,
            resistanceLevel: protocolId === "conconi-bike" && typeof config.initialPowerWatts !== "number" && typeof config.initialResistanceLevel === "number"
...
function getStageLoad(protocol, stage) {
    if (protocol.environment === "esteira")
        return { value: stage.speedKmh, unit: "km/h" };
    if (protocol.environment === "bicicleta") {
        if (typeof stage.powerWatts === "number")
            return { value: stage.powerWatts, unit: "w" };
        return { value: stage.resistanceLevel, unit: "nivel" };
    }
    return { value: undefined, unit: "km/h" };
}
function detectConconiDeflection(protocol, stages, status) {
    ...
    let detectionStatus = "inconclusivo";
    let quality = "baixa";
    ...
    return {
        ...
        quality,
        method: "Regressão linear segmentada: compara uma reta única com duas retas e exige melhora de ajuste e redução de inclinação para aceitar deflexão.",
        singleLineR2: round(single.r2, 3),
        ...
        message: detectionStatus === "inconclusivo"
...
function calculateExternalVo2(execution, ageYears, sex, weightKg) {
    ...
    if (execution.protocolId === "rockport-1mile" &&
        ...
        return round(132.853 -
            ...
            0.1565 * external.heartRateEnd, 1);
    }
    return undefined;
}
function validateExternal(protocol, execution, errors, warnings) {
    const external = execution.external ?? {};
    protocol.fields.forEach((field) => {
        const value = external[field.id];
        ...
        if (typeof value === "number") {
            ...
            if (typeof field.max === "number" && value > field.max)
                warnings.push(`${field.label} fora da faixa esperada; confira.`);
        }
    });
}
function comparePrevious(current, previous) {
    ...
    if (typeof currentValue !== "number" || typeof previousValue !== "number")
        return undefined;
    const compatible = current.protocolId === previous.protocolId &&
...
function calculateCardioProtocolSnapshot(execution, context = {}, previous) {
    ...
    const maxHeartRateObserved = Math.max(0, ...validStages
        .flatMap((stage) => [stage.heartRateMax, stage.heartRateEnd, stage.heartRateAvg])
        .filter((value) => typeof value === "number"));
    const maxLoadObserved = Math.max(0, ...validStages
    ...
    if (execution.status === "interrompido")
        warnings.push("Teste interrompido: salvar como incompleto e não apresentar como resultado válido.");
    ...
    if (protocol.environment === "externo" || execution.protocolId === "custom-cardio") {
        ...
        if (execution.protocolId === "rockport-1mile" &&
            ...
            if (typeof context.ageYears !== "number")
                errors.push("Idade do aluno é obrigatória para calcular Rockport.");
            if (typeof context.weightKg !== "number")
                errors.push("Peso do aluno é obrigatório para calcular Rockport.");
            if (!context.sex)
                errors.push("Sexo biológico é obrigatório para calcular Rockport.");
        }
    ...
    if (execution.protocolId === "conconi-treadmill" || execution.protocolId === "conconi-bike") {
        if (validStages.length < 6)
            warnings.push("Conconi requer pelo menos 6 estágios válidos para análise automática.");
    ...
    const recoveryDrop1Min = typeof execution.recovery?.immediateBpm === "number" && typeof execution.recovery?.after1MinBpm === "number"
