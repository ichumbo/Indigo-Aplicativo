...
exports.FUNCTIONAL_BATTERY_TEMPLATES = exports.FUNCTIONAL_TEST_CATALOG = exports.FUNCTIONAL_CATEGORIES = void 0;
exports.getFunctionalTestDefinition = getFunctionalTestDefinition;
exports.createFunctionalExecution = createFunctionalExecution;
exports.createCustomFunctionalDefinition = createCustomFunctionalDefinition;
exports.createCustomFunctionalExecution = createCustomFunctionalExecution;
exports.validateFunctionalScreening = validateFunctionalScreening;
exports.calculateFunctionalTestSnapshot = calculateFunctionalTestSnapshot;
exports.buildFunctionalBatteryFromTemplate = buildFunctionalBatteryFromTemplate;
...
const bilateralNumber = (id, label, unit, help, min, max) => ({
    id,
    label,
    kind: "number",
    unit,
...
const numberField = (id, label, unit, help, min, max, attempts = 3) => ({
    id,
    label,
    kind: "number",
    unit,
...
exports.FUNCTIONAL_TEST_CATALOG = [
    {
...
exports.FUNCTIONAL_BATTERY_TEMPLATES = [
    {
        id: "geral",
...
exports.FUNCTIONAL_TEST_CATALOG.push({
    id: "functional-reach",
...
function getFunctionalTestDefinition(testId, customDefinition) {
    if (customDefinition)
        return customDefinition;
    return exports.FUNCTIONAL_TEST_CATALOG.find((test) => test.id === testId);
}
function createFunctionalExecution(testId, order, required = false) {
    const definition = getFunctionalTestDefinition(testId);
    if (!definition)
        throw new Error("Teste funcional não encontrado.");
...
function createCustomFunctionalDefinition(input) {
    ...
    return {
        ...
        professionalRestriction: "personal",
    };
}
function createCustomFunctionalExecution(definition, order) {
    const now = new Date().toISOString();
    return {
        id: createId("functional"),
        testId: definition.id,
...
function validateFunctionalScreening(screening) {
    const alerts = [];
    if (!screening?.consentAccepted)
        alerts.push("Consentimento do aluno não registrado.");
    if (!screening?.parqReviewed)
        alerts.push("Questionário de prontidão não revisado.");
    if (screening?.cardiovascularSymptoms)
        alerts.push("Sintomas cardiovasculares relatados: não recomendar continuidade automática.");
    if (screening?.dizziness)
        alerts.push("Tontura relatada: adiar ou encaminhar conforme avaliação profissional.");
    if (screening?.balanceLoss)
        alerts.push("Perda de equilíbrio relatada: adaptar, adiar ou encaminhar.");
    if (screening?.importantMovementLimitation)
        alerts.push("Limitação importante de movimento registrada.");
    if (screening?.medicalClearanceNeeded)
        alerts.push("Liberação médica indicada antes de testes exigentes.");
...
function consolidate(attempts, strategy) {
    ...
    if (strategy === "sum")
        return round(values.reduce((sum, value) => sum + value, 0), 1);
    if (strategy === "best" || strategy === "manual" || strategy === "epley-1rm")
        return round(Math.max(...values), 1);
    return round(values[0], 1);
}
function getPrimaryField(definition) {
    return definition.fields.find((field) => field.id === definition.primaryFieldId);
}
function getPrimaryResult(definition, execution) {
    ...
    if (primaryField.bilateral) {
        ...
        if (typeof right === "number" && typeof left === "number")
            return round((right + left) / 2, 1);
        return right ?? left;
    }
    if (result?.attempts?.length)
        return consolidate(result.attempts, definition.resultStrategy);
    return typeof result?.value === "number" ? result.value : undefined;
}
function calculateAsymmetry(definition, execution) {
    ...
    return {
        ...
        largerSide: right > left ? "direito" : left > right ? "esquerdo" : "igual",
        formula: "diferença absoluta = |direito - esquerdo|; diferença percentual = diferença / maior valor x 100",
    };
}
function calculateEpleyOneRm(execution) {
    ...
    if (typeof load !== "number" || typeof reps !== "number" || reps <= 0)
        return undefined;
...
function calculateFunctionalTestSnapshot(execution, previous) {
    ...
    definition.fields.forEach((field) => {
        const result = execution.fields[field.id];
        if (!field.required || execution.status === "adiado" || execution.status === "contraindicado")
            return;
        if (field.bilateral) {
            if (validAttemptValues(result?.right).length === 0)
                errors.push(`${field.label} direito é obrigatório.`);
            if (validAttemptValues(result?.left).length === 0)
                errors.push(`${field.label} esquerdo é obrigatório.`);
            return;
        }
        if (field.kind === "number" && validAttemptValues(result?.attempts).length === 0 && typeof result?.value !== "number") {
            errors.push(`${field.label} é obrigatório.`);
        }
        if (field.kind === "text" && !String(result?.value ?? "").trim())
            errors.push(`${field.label} é obrigatório.`);
        if (field.kind === "boolean" && typeof result?.value !== "boolean")
            warnings.push(`${field.label} não foi marcado.`);
    });
    Object.entries(execution.fields).forEach(([fieldId, result]) => {
        ...
        values.forEach((value) => {
            if (typeof field.min === "number" && value < field.min)
                errors.push(`${field.label} abaixo do limite mínimo.`);
            if (typeof field.max === "number" && value > field.max)
                warnings.push(`${field.label} acima da faixa esperada; confira a medida.`);
        });
    });
    const primaryField = getPrimaryField(definition);
...
function buildFunctionalBatteryFromTemplate(templateId, currentCount = 0) {
    const template = exports.FUNCTIONAL_BATTERY_TEMPLATES.find((item) => item.id === templateId);
    if (!template)
        throw new Error("Modelo de bateria não encontrado.");
