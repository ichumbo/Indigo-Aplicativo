...
exports.ANAMNESIS_STEPS = exports.ANAMNESIS_STATUS_OPTIONS = exports.STUDENT_STATUS_OPTIONS = void 0;
exports.getStudentProfile = getStudentProfile;
exports.listStudentProfilesForTrainer = listStudentProfilesForTrainer;
...
exports.saveStudentRegistration = saveStudentRegistration;
exports.updateStudentStatus = updateStudentStatus;
exports.generateAnamnesisInvite = generateAnamnesisInvite;
exports.sendAnamnesisReminder = sendAnamnesisReminder;
exports.cancelAnamnesisInvite = cancelAnamnesisInvite;
exports.markAnamnesisReviewed = markAnamnesisReviewed;
exports.requestAnamnesisUpdate = requestAnamnesisUpdate;
exports.revokeStudentSessions = revokeStudentSessions;
exports.buildChecklistFromAnswers = buildChecklistFromAnswers;
exports.getLatestAnamnesisVersion = getLatestAnamnesisVersion;
exports.getStudentStatusLabel = getStudentStatusLabel;
exports.getAnamnesisStatusLabel = getAnamnesisStatusLabel;
exports.formatProfileDate = formatProfileDate;
exports.formatProfileDateTime = formatProfileDateTime;
exports.daysSince = daysSince;
exports.daysUntil = daysUntil;
exports.formatRelativeDayCount = formatRelativeDayCount;
exports.calculateAdherence = calculateAdherence;
exports.calculateFollowUpDuration = calculateFollowUpDuration;
exports.calculateSetVolume = calculateSetVolume;
exports.buildLoadEvolutionInsights = buildLoadEvolutionInsights;
exports.buildStudentAlerts = buildStudentAlerts;
exports.resetStudentProfileStoreForTests = resetStudentProfileStoreForTests;
const async_storage_1 = __importDefault(require("@react-native-async-storage/async-storage"));
const feedback_store_1 = require("@/services/feedback-store");
const subscription_service_1 = require("@/services/subscription-service");
exports.STUDENT_STATUS_OPTIONS = [
...
exports.ANAMNESIS_STATUS_OPTIONS = [
...
function createDefaultAnamnesisVersion() {
    ...
    return {
        ...
        answeredBy: feedback_store_1.DEMO_STUDENT.id,
...
function createDefaultProfile() {
    ...
    return {
        ...
        registration: {
            ...
            updatedAt: now,
            updatedBy: feedback_store_1.DEMO_TRAINER.id,
        },
        status: "ativo",
        followUp: {
        ...
        updatedAt: now,
        audit: [makeAudit("created", feedback_store_1.DEMO_TRAINER.id, "trainer", "Perfil inicial criado no armazenamento local.")],
    };
}
function createDefaultExecutedSets() {
    const squatEquipment = {
...
async function readState() {
    ...
    if (!stored) {
        ...
        await async_storage_1.default.setItem(STORAGE_KEY, JSON.stringify(initialState));
        return initialState;
    }
    try {
        const parsed = JSON.parse(stored);
        const profiles = parsed.profiles ?? {};
        if (!profiles[feedback_store_1.DEMO_STUDENT.id]) {
...
function hasProfilePermission(profile, requesterId, role) {
    if (role === "admin")
        return true;
    if (role === "student")
...
function requireProfilePermission(profile, requesterId, role) {
    if (!hasProfilePermission(profile, requesterId, role)) {
        throw new Error("Voce nao tem permissao para acessar este perfil.");
    }
}
async function getStudentProfile(studentId = feedback_store_1.DEMO_STUDENT.id, requesterId = feedback_store_1.DEMO_TRAINER.id, role = "trainer") {
...
async function listStudentProfilesForTrainer(trainerId = feedback_store_1.DEMO_TRAINER.id, requesterId = trainerId, role = "trainer") {
    const state = await readState();
...
async function createStudentProfile(input, actorId = input.trainerId, actorRole = "trainer") {
    ...
    const currentTrainerStudents = Object.values(state.profiles).filter((p) => p.trainerId === input.trainerId && p.status === "ativo");
    const entitlementCheck = await (0, subscription_service_1.validateStudentAdditionAllowed)(input.trainerId, currentTrainerStudents.length);
    if (!entitlementCheck.allowed) {
        throw new Error(entitlementCheck.reason ||
    ...
    if (email &&
        Object.values(state.profiles).some((profile) => profile.trainerId === input.trainerId &&
    ...
    const profile = {
        ...
        updatedAt: now,
        audit: [
            makeAudit("created", actorId, actorRole, "Aluno cadastrado pelo personal."),
        ],
    };
    await writeState({
        profiles: {
...
function getWhatsAppUrl(value, prefilledMessage) {
    const digits = normalizePhone(value);
    if (digits.length < 10)
        return null;
    const withCountry = digits.startsWith("55") ? digits : `55${digits}`;
    if (!/^\d{12,13}$/.test(withCountry))
        return null;
...
function validateRegistration(registration) {
    ...
    if (!age || age > 120)
        errors.push("Informe uma data de nascimento valida.");
...
function getChangedRegistrationFields(previous, next) {
    const changed = [];
    const directFields = [
        "fullName",
        "avatar",
    ...
    Object.keys(next.contact).forEach((field) => {
        if ((previous.contact[field] ?? "") !== (next.contact[field] ?? ""))
            changed.push(`contact.${field}`);
    });
    return changed;
}
async function saveStudentRegistration(studentId, registration, actorId = feedback_store_1.DEMO_TRAINER.id, actorRole = "trainer") {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
    ...
    const sanitized = {
        ...
        administrativeNotes: actorRole === "student"
    ...
    const updatedProfile = {
        ...
        updatedAt: now,
        audit: [
            makeAudit("registration_updated", actorId, actorRole, changedFields.length > 0
...
async function updateStudentStatus(studentId, nextStatus, actorId = feedback_store_1.DEMO_TRAINER.id, actorRole = "trainer") {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
    ...
    const updatedProfile = {
        ...
        updatedAt: now,
        audit: [
            makeAudit("status_changed", actorId, actorRole, `Status alterado para ${getStudentStatusLabel(nextStatus)} sem apagar historico.`),
            ...profile.audit,
        ],
    };
    await writeState({
...
async function generateAnamnesisInvite(studentId, actorId = feedback_store_1.DEMO_TRAINER.id, dueAt = dateDaysFromNow(7)) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
    ...
    const updatedProfile = {
        ...
        updatedAt: now,
        audit: [
            makeAudit("anamnesis_invite_created", actorId, "trainer", "Convite de anamnese gerado com validade e sem dados pessoais na URL."),
            ...profile.audit,
        ],
    };
    await writeState({
...
async function sendAnamnesisReminder(studentId, actorId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
...
async function cancelAnamnesisInvite(studentId, actorId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
...
async function markAnamnesisReviewed(studentId, trainerNote, actorId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
    ...
    if (!latestVersion)
        throw new Error("Nao ha anamnese para revisar.");
    const now = new Date().toISOString();
    ...
    const updatedProfile = {
        ...
        updatedAt: now,
        audit: [
            makeAudit("anamnesis_reviewed", actorId, "trainer", "Anamnese revisada preservando respostas originais e observacoes do treinador."),
            ...profile.audit,
        ],
    };
    await writeState({
...
async function requestAnamnesisUpdate(studentId, actorId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
...
async function revokeStudentSessions(studentId, actorId = feedback_store_1.DEMO_TRAINER.id) {
    const state = await readState();
    const profile = state.profiles[studentId];
    if (!profile)
        throw new Error("Perfil do aluno nao encontrado.");
...
function buildChecklistFromAnswers(answers) {
    return answers
...
function getLatestAnamnesisVersion(profile) {
    return profile.anamnesis.versions[0];
}
function getStudentStatusLabel(status) {
    return exports.STUDENT_STATUS_OPTIONS.find((option) => option.value === status)?.label ?? status;
}
function getAnamnesisStatusLabel(status) {
    return exports.ANAMNESIS_STATUS_OPTIONS.find((option) => option.value === status)?.label ?? status;
}
...
function formatRelativeDayCount(days, mode) {
    if (days === null)
        return "Sem registro";
    if (days === 0)
        return mode === "since" ? "Hoje" : "Hoje";
    if (mode === "since")
        return `${days} dia${days === 1 ? "" : "s"} atras`;
    if (days < 0)
        return `${Math.abs(days)} dia${Math.abs(days) === 1 ? "" : "s"} vencido`;
    return `em ${days} dia${days === 1 ? "" : "s"}`;
}
function calculateAdherence(frequency) {
    if (frequency.plannedSessions <= 0)
        return 0;
    return Math.round((frequency.completedSessions / frequency.plannedSessions) * 100);
}
function calculateFollowUpDuration(startedAt) {
...
function calculateSetVolume(set) {
    if (!set.validForProgression || set.warmup || !set.completed)
        return null;
    if (set.loadUnit !== "kg")
        return null;
    if (typeof set.executedLoad !== "number" || typeof set.executedReps !== "number")
        return null;
    return set.executedLoad * set.executedReps;
}
function buildLoadEvolutionInsights(profile) {
    ...
    return [...grouped.entries()]
        ...
        const volumes = records.map((record) => record.volume).filter((value) => typeof value === "number");
        const loads = records.map((record) => record.load).filter((value) => typeof value === "number");
        ...
        const progressPercent = firstVolume && lastVolume ? Math.round(((lastVolume - firstVolume) / firstVolume) * 100) : undefined;
        const hasPainAlert = sorted.some((set) => Boolean(set.pain));
        let trendLabel = "Dados insuficientes para tendencia";
        let dataQuality = compatibleRecords >= 3 ? "compatible" : "insufficient";
        if (compatibleRecords >= 3 && firstVolume && lastVolume) {
            if (lastVolume > firstVolume)
                trendLabel = "Evolucao compativel";
            else if (lastVolume < firstVolume)
                trendLabel = "Queda recente";
            else
                trendLabel = "Estavel";
        }
...
function buildStudentAlerts(profile) {
    const alerts = [];
...
async function resetStudentProfileStoreForTests() {
